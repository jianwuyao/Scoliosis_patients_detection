...
def get_pose_estimation_prediction(pose_model, image, centers, scales, transform):
    ...
    coords, _ = get_final_preds(cfg, output.cpu().detach().numpy(), np.asarray(centers), np.asarray(scales))
    return coords
...
def prepare_output_dirs(prefix):
    pose_dir = os.path.join(prefix, "pose_video")
    if os.path.exists(pose_dir) and os.path.isdir(pose_dir):
        shutil.rmtree(pose_dir)  # 递归删除
...
def main():
    ...
    outcap = cv2.VideoWriter('{}/{}_pose.avi'.format(pose_dir, os.path.splitext(os.path.basename(args.videoFile))[0]),
    ...
    while vidcap.isOpened():
        ...
        if args.writeBoxFrames:
            for box in pred_boxes:
                cv2.rectangle(image_debug, box[0], box[1], color=(0, 255, 0), thickness=3)  # green
        ...
        new_csv_row = [count, len(pred_boxes)]
        for coords in pose_preds:
            # Draw each point or skeleton on image
            assert coords.shape == (NUM_KPTS, 2)  # the shape should be equal to [17,2]
            for i in range(len(SKELETON)):
                kpt_a, kpt_b = SKELETON[i][0], SKELETON[i][1]
                x_a, y_a = coords[kpt_a][0], coords[kpt_a][1]
                x_b, y_b = coords[kpt_b][0], coords[kpt_b][1]
                x_coord, y_coord = coords[i][0], coords[i][1]
                cv2.circle(image_debug, (int(x_coord), int(y_coord)), radius=6, color=CocoColors[i], thickness=-1)
                if args.drawSkeletons:
                    cv2.line(image_debug, (int(x_a), int(y_a)), (int(x_b), int(y_b)), CocoColors[i], 2)
                new_csv_row.extend([x_coord, y_coord])

        total_then = time.time()
        csv_output_rows.append(new_csv_row)

        fps = 1 / (total_then - total_now)  # 每帧推理时间将是0.26秒，接近4fps，无法实现实时
        cv2.putText(image_debug, 'FPS: ' + "%.2f" % fps, (100, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        cv2.imshow("pose", image_debug)
        img_file = os.path.join(pose_dir, 'pose_{:08d}.jpg'.format(count))
        cv2.imwrite(img_file, image_debug)
        outcap.write(image_debug)  # 输出avi

        if cv2.waitKey(1) & 0xFF == ord('q'):  # 每1ms检查键盘输入，按q即可退出pose窗口
            break

    # write csv
    csv_output_filename = os.path.join(pose_dir, 'pose_coord_data.csv')
    csv_headers = ['count', 'person_num']
