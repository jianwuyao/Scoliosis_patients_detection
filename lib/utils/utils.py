...
def create_logger(cfg, cfg_name, phase='train'):
    # creat OUTPUT_DIR and LOG_DIR
    root_output_dir, root_log_dir = Path(cfg.OUTPUT_DIR), Path(cfg.LOG_DIR)
    if not root_output_dir.exists():
        root_output_dir.mkdir()
        root_log_dir.mkdir()

    # creat final_output_dir
    ...
    # set up logger and creat tensorboard_log_dir
...
def get_model_summary(model, *input_tensors, item_length=26, verbose=False):
    ...
    for layer in summary:
        ...
        if verbose:
            details += "{}{}{}{}{}{}{}{}{}{}".format(
                layer.name, ' ' * (space_len - len(layer.name)),
                layer.input_size, ' ' * (space_len - len(str(layer.input_size))),
                layer.output_size, ' ' * (space_len - len(str(layer.output_size))),
                layer.num_parameters, ' ' * (space_len - len(str(layer.num_parameters))),
                layer.multiply_adds, ' ' * (space_len - len(str(layer.multiply_adds)))) \
                       + os.linesep + '-' * space_len * 5 + os.linesep
