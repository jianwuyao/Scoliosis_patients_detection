...
def train(config, train_loader, model, criterion, optimizer, epoch, output_dir,
          final_log_dir, writer_dict):
    """ 训练 """
    batch_time = AverageMeter()  # 计算并存储平均值和当前值
...
def validate(config, val_loader, val_dataset, model, criterion, output_dir,
             final_log_dir, writer_dict=None):
    """ 测试 """
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            ...
            all_boxes[idx:idx + num_images, 4] = np.prod(s*200, 1)  # 面积
            all_boxes[idx:idx + num_images, 5] = score  # 得分
...
def _print_name_value(name_value, full_arch_name):
    ...
    logger.info(
        '| ' + full_arch_name + ' ' + ' '.join(['| {:.3f}'.format(value) for value in values]) + ' |'
    )
