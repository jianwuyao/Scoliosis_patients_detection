...
def main():
    ...
    logger, final_output_dir, final_log_dir = create_logger(cfg, args.cfg, 'train')
    # logger.info(pprint.pformat(args))  # 以字符串形式保存参数
    # logger.info(cfg)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        pin_memory=cfg.PIN_MEMORY  # 锁页内存：内存的Tensor转义到GPU的显存会更快
    )
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        train(cfg, train_loader, model, criterion, optimizer, epoch,
              final_output_dir, final_log_dir, writer_dict)
