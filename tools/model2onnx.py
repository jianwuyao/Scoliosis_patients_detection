...
box_model.eval()
box_input = []
image_rgb = cv2.cvtColor(cv2.imread('./box_model_input.jpg'), cv2.COLOR_BGR2RGB)  # 输入任意一张真实图像
...
torch.onnx.export(box_model, box_input, "fasterrcnn_resnet50_fpn.onnx",
                  input_names=['input'], output_names=['boxes', 'labels', 'scores'], opset_version=11)

pose_model = eval('lib.models.pose_hrnet.get_pose_net')(cfg, is_train=False)  # 姿态估计模型
...
torch.onnx.export(pose_model, pose_input, "pose_hrnet_256.onnx",
...
"""
    验证ONNX图的有效性
"""
onnx_box_model = onnx.load("fasterrcnn_resnet50_fpn.onnx")
onnx.checker.check_model(onnx_box_model)
onnx_pose_model = onnx.load("pose_hrnet_256.onnx")
...
"""
ort_box_session = onnxruntime.InferenceSession("fasterrcnn_resnet50_fpn.onnx")
ort_pose_session = onnxruntime.InferenceSession("pose_hrnet_256.onnx")
